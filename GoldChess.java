...
public class GoldChess extends Chess{
    ...
    public GoldChess clone() {
        GoldChess output = new GoldChess(false);
    ...
    public GoldChess move(String... moves) {
    ...
    public Map<String, Integer> perftMap(int depth) {
        ...
        for (short move : moves) {
            Thread temp = new Thread() {
                public void run() {
                    int divide = GoldChess.this.clone().moveNoUpdate(move).perft(depth - 1);
                    nodes[0] += divide;
    ...
    private GoldChess moveNoUpdate(short move) {
    ...
    private void makeEnPassantMove(short move) {
        long start = getStartingSquare(move);
        long end = getEndingSquare(move);
        long captureSquare = GoldChess.compassLong(end, end - start > 0 ? SOUTH : NORTH, 0);
